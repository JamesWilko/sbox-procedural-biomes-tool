HEADER
{
	Description = "Terrain Biome Preview";
    DevShader = true;
    DebugInfo = false;
}
...
PS
{
	CreateTexture2D( g_tTexture ) < Attribute( "Texture" ); SrgbRead( false ); Filter( BILINEAR ); AddressU( WRAP ); AddressV( WRAP ); >;
	float g_flRadius < Attribute( "Radius" ); Default( 16.0f ); >;
	float g_flOpacity < Attribute( "Opacity" ); Default( 0.5f ); >;
	...
	float4 MainPs( PixelInput i ) : SV_Target0
	{
		...
		if ( InDecalBounds( vPositionWs, i.DecalOrigin, i.DecalRight, i.DecalUp, i.DecalForward ) )
		{
			...
			float4 tex = Tex2D( g_tTexture, uv ).rgba;
			float4 color = float4( tex.rgb, tex.a * g_flOpacity );
			return color;
		}
